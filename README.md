...
3. **Anti-AFK**: Triggers only when CS2 is active and your cursor is hidden (actively in-game). Simulates slight random movement keystrokes (`W`, `A`, `S`, `D` with opposing corrections).
...
2. The GUI window appears with feature toggles and a log display.
3. Start CS2.
4. **Game Settings Requirements**:
   - Run CS2 in **Windowed** or **Borderless Windowed** mode (so the tool can capture screenshots).
   - Use default/standard HUD color theme.
5. **Calibration (Highly Recommended)**:
   - Hover your mouse over the **PLAY** button in CS2 and press **F9** (or click Calibrate in the GUI). Note the relative coordinates printed in the log.
...
6. Click **Start** in the GUI (or press **F10**) to toggle the assistant on/off.
7. Toggle individual features with the checkboxes (Auto-Accept, Auto-Queue, Anti-AFK).
8. Press **F11** (or close the window) to exit the assistant.
